"""Reprojection: resample masters shot at different pixel scales onto one
common pixel grid, via WCS.
...
binning-grid misalignment, per the design review).
...
rgbcomp -> GraXpert -> SPCC -> THEN reproject the processed RGB composite
onto L's grid (this module) -> GHT stretch both -> rgbcomp -lum=.
Multi-channel (already-composited RGB) input is supported here
specifically so reprojection can run this late in the chain.
"""
